...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    ### TODO
    apply_dense = dense_layer(output)
    apply_dropout = dropout(apply_dense)
    residual = apply_dropout + input
    return ln_layer(residual)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    #Create the multi head attention layer by using the self variable that calls our prev code
    mha_out = self.self_attention(hidden_states, attention_mask)
    #mha = self.self_attention
    # key_layer = mha.transform(hidden_states, mha.key)
    # value_layer = mha.transform(hidden_states, mha.value)
    # query_layer = mha.transform(hidden_states, mha.query)
    # mha_out = mha.attention(key_layer, value_layer, query_layer, attention_mask)
    #Use the add norm code from before on the MHA code
    add_norm_MHA = self.add_norm(hidden_states, mha_out, self.attention_dense, self.attention_dropout, self.attention_layer_norm)

    #Add the feed forward layer
    ff_out = self.interm_af(self.interm_dense(add_norm_MHA))

    #Use the add norm code but now for after feed forward completed
    add_norm_FF = self.add_norm(add_norm_MHA, ff_out, self.out_dense, self.out_dropout, self.out_layer_norm)

    return add_norm_FF
  
    # self_attention_output = self.self_attention(hidden_states, attention_mask)

    # # Apply add-norm for multi-head attention
    # hidden_states = self.add_norm(hidden_states, self_attention_output, self.attention_dense,
    #                                   self.attention_dropout, self.attention_layer_norm)

    # # Feed forward layer
    # intermediate_output = self.interm_af(self.interm_dense(hidden_states))

    # # Apply add-norm for feed forward layer
    # hidden_states = self.add_norm(hidden_states, intermediate_output, self.out_dense,
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    # Add three embeddings together; then apply embed_layer_norm and dropout and return.
    embedded_output = inputs_embeds + pos_embeds + tk_type_embeds
    embedded_output = self.embed_layer_norm(embedded_output)
    embedded_output = self.embed_dropout(embedded_output)
